...
func TestAuthService_Login(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			req: &authpb.LoginRequest{
				AuthData: &authpb.AuthData{Login: nil, Password: []byte("p")},
			},
			wantErr:  true,
			wantCode: codes.InvalidArgument,
		},
		{
			...
			req: &authpb.LoginRequest{
				AuthData: &authpb.AuthData{Login: []byte(keyDummyUserID), Password: nil},
			},
			wantErr:  true,
			wantCode: codes.InvalidArgument,
		},
		{
			...
			req: &authpb.LoginRequest{
				AuthData: &authpb.AuthData{Login: []byte(keyDBFail), Password: []byte("p")},
			},
			wantErr:  true,
		...
		},
		{
			...
			req: &authpb.LoginRequest{
				AuthData: &authpb.AuthData{
					Login:    []byte(keySessionFail),
					Password: []byte("very-long-dummy-bytes"),
				},
			},
			wantErr:  true,
...
func TestAuthService_Logout(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			req: &authpb.LogoutRequest{
				RefreshToken: &authpb.RefreshToken{RefreshToken: []byte("")},
			},
		...
		},
		{
			...
			req: &authpb.LogoutRequest{
				RefreshToken: &authpb.RefreshToken{RefreshToken: []byte("refresh-ok")},
			},
			wantErr: false,
		},
		{
			...
			req: &authpb.LogoutRequest{
				RefreshToken: &authpb.RefreshToken{RefreshToken: []byte("revoke-fail")},
			},
			wantErr: true,
		},
	}
...
func TestAuthService_Register(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "nil login",
			req: &authpb.RegisterRequest{
				AuthData: &authpb.AuthData{Login: nil, Password: []byte("pass")},
			},
			wantErr:  true,
			wantCode: codes.InvalidArgument,
		},
		{
			name: "nil password",
			req: &authpb.RegisterRequest{
				AuthData: &authpb.AuthData{Login: []byte("user"), Password: nil},
			},
			wantErr:  true,
			wantCode: codes.InvalidArgument,
		},
		{
			...
			req: &authpb.RegisterRequest{
				AuthData: &authpb.AuthData{
					Login: []byte("already-exists"), Password: []byte("pass")},
			},
			wantErr:  true,
		...
		},
		{
			...
			req: &authpb.RegisterRequest{
				AuthData: &authpb.AuthData{
					Login: []byte("create-fail"), Password: []byte("pass")},
			},
			wantErr:  true,
			wantCode: codes.Internal,
		},
		{
			...
			req: &authpb.RegisterRequest{
				AuthData: &authpb.AuthData{
					Login:    []byte("session-fail-register"),
					Password: []byte("pass"),
				},
			},
			wantErr:  true,
		...
		},
		{
			...
			req: &authpb.RegisterRequest{
				AuthData: &authpb.AuthData{
					Login: []byte("new-user"), Password: []byte("pass")},
			},
			wantErr:  false,
			wantCode: codes.OK,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctxTO, cancel := context.WithTimeout(
				context.Background(), model.RepoOperationTO)
