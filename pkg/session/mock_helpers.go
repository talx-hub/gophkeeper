...
func (b *storageMockBuilder) WithSave() *storageMockBuilder {
	b.storage.EXPECT().
		...
		RunAndReturn(func(
			_ context.Context, tokenHash []byte, userID model.UserID, expiresAt time.Time) error {
			if userID == "save-fail" {
...
func (b *storageMockBuilder) WithValidate() *storageMockBuilder {
	b.storage.EXPECT().
		...
		RunAndReturn(func(_ context.Context, _ []byte, userID model.UserID) error {
			if userID == "validate-fail" {
...
func (b *storageMockBuilder) WithDelete() *storageMockBuilder {
	b.storage.EXPECT().
		...
		RunAndReturn(func(_ context.Context, tokenHash []byte) error {
			if bytes.Equal(tokenHash, []byte("delete-fail")) {
