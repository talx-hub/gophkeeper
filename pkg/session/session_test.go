...
func TestManager_RefreshSession(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		access, refresh, err := m.RefreshSession(context.Background(),
			"user-1", []byte("old-token"))
		require.NoError(t, err)
		require.NotEmpty(t, access)
		require.NotEmpty(t, refresh)
	})
...
func TestManager_ValidateRefreshToken(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		m := NewManager(st, issuer)
		err := m.ValidateRefreshToken(context.Background(),
			[]byte("token-ok"), "user-ok")
		require.NoError(t, err)
	})
	...
	t.Run("validate fails", func(t *testing.T) {
		...
		m := NewManager(st, issuer)
		err := m.ValidateRefreshToken(context.Background(),
			[]byte("validate-fail"), "validate-fail")
		require.Error(t, err)
	})
}
...
func TestManager_RevokeSession(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		st := newStorageMock(t).WithDelete().Build()
		m := NewManager(st, issuer)
		err := m.RevokeSession(context.Background(), []byte("token-ok"))
		require.NoError(t, err)
	})
	...
	t.Run("delete fails", func(t *testing.T) {
		st := newStorageMock(t).WithDelete().Build()
		m := NewManager(st, issuer)
		err := m.RevokeSession(context.Background(), []byte("delete-fail"))
		require.Error(t, err)
	})
}
